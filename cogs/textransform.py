...
############################# E T I R E U R   D E   T E X T E ############################
    ...
    async def etirer(self,ctx,*,texte):
        ...
        if not texte:
            await ctx.send("Je n'ai pas eu de texte à étirer. Réessaye",delete_after=5)
        else:
...
############################ BULLES TEXTE ############################
    ...
    async def arrondir(self,ctx,*,texte):
        await ctx.message.delete(delay=1.0)
        if not texte:
            await ctx.send("Je n'ai pas eu de texte à arrondir. Réessaye",delete_after=5)
        else:
...
############################ CARRE TEXTE ############################
    ...
    async def carre(self,ctx,*,texte):
        await ctx.message.delete(delay=1.0)
        if not texte:
            await ctx.send("Je n'ai pas eu de texte en argument. Réessaye",delete_after=5)
        else:
            chars = '🄰🄱🄲🄳🄴🄵🄶🄷🄸🄹🄺🄻🄼🄽🄾🄿🅀🅁🅂🅃🅄🅅🅆🅇🅈🅉'
            final = []
            for mot in texte:
...
############################ MINUSCULES TEXTE ############################
    ...
    async def minuscule(self,ctx,*,texte):
        ...
        if texte == "":
            ...
            chars = 'ᴀʙᴄᴅᴇꜰɢʜɪᴊᴋʟᴍɴᴏᴘQʀꜱᴛᴜᴠᴡxʏᴢ'
            final = []
            for mot in texte:
                for lettres in mot:
                    if lettres.isalpha():
...
############################ MAJUSCULES TEXTE ############################
    ...
    async def cursive(self,ctx,*,texte):
        ...
        if not texte:
            ...
            chars = '𝒜ℬ𝒞𝒟ℰℱ𝒢ℋℐ𝒥𝒦ℒℳ𝒩𝒪𝒫𝒬ℛ𝒮𝒯𝒰𝒱𝒲𝒳𝒴𝒵'
            final = []
            for mot in texte:
                for lettres in mot:
                    if lettres.isalpha():
